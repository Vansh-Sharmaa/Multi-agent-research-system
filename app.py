...
if st.session_state.running and not st.session_state.done:
    ...
    try:
        ...
        # If it fails completely after all 5 retries, catch it so the app doesn't crash
        if hasattr(e, 'last_attempt') and e.last_attempt is not None:
            actual_err = e.last_attempt.exception()
            st.error(f"Pipeline failed due to persistent API errors: {repr(actual_err)}")
            print("UNDERLYING ERROR:", repr(actual_err))
        else:
            st.error(f"Pipeline failed due to persistent API errors: {e}")
